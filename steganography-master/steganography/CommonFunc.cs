...
namespace steganography.Functions
{
    public static class CommonFunc
    {
        ...
        public static double calculatePSNR(byte[] arr, byte[] new_arr, Colours channel)
        {
            double PSNR = 0;
            for (int i = 0; i < arr.Length; i++)
            {
                PSNR += Math.Pow(arr[i] - new_arr[i], 2);
            }
            double tmp = 512 * 512 * Math.Pow(256 - 1, 2);
            PSNR = 10 * Math.Log10(tmp / PSNR);
